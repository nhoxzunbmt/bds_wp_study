...
class Currency_Prices {
	 ...
	 */
	public function __construct( $estate, $currency ) {
		$this->estate   = $estate;
	...
	private function set_prices() {
		...
		foreach ( $price_keys as $price_key ) {
			$price_value = floatval( get_field( 'estate_attr_' . $price_key, $this->estate->get_ID() ) );
		...
		foreach ( $offer_types as $offer_type ) {
			...
			foreach ( $price_keys as $price_key ) {
				$price_value = floatval( get_field( 'estate_attr_' . $price_key . '_offer_' . $offer_type->get_ID(), $this->estate->get_ID() ) );
