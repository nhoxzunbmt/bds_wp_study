...
class Text_Attribute extends Attribute {
	...
	public function set_text_attribute_data() {
		...
		$has_archive                         = get_field( $this->get_slug() . '_has_archive', 'option' );
		$this->attribute_data['has_archive'] = is_null( $has_archive ) ? true : ! empty( $has_archive );

		$default_values                         = get_field( $this->get_slug() . '_default_values', 'option' );
		$this->attribute_data['default_values'] = empty( $default_values ) ? array() : $default_values;

		$most_popular_limit                         = get_field( $this->get_slug() . '_most_popular_limit', 'option' );
		$this->attribute_data['most_popular_limit'] = intval( $most_popular_limit );

		$tags                         = get_field( $this->get_slug() . '_tags', 'option' );
		$this->attribute_data['tags'] = ! empty( $tags );

		$new_box                         = get_field( $this->get_slug() . '_new_box', 'option' );
		$this->attribute_data['new_box'] = ! is_null( $new_box ) && ! empty( $new_box );

		$new_box_independent                         = get_field( 'myhome_' . $this->get_slug() . '_new_box_independent', 'option' );
		$this->attribute_data['new_box_independent'] = ! empty( $new_box_independent );

		$parent_id                         = get_field( $this->get_slug() . '_parent_id', 'option' );
		$this->attribute_data['parent_id'] = intval( $parent_id );
	...
	public function get_estate_values( $estate_id ) {
		$values = array();
		$terms  = Term_Factory::get_from_estate( $estate_id, $this );
	...
	public function get_parent_type() {
		$parent_type = get_field( $this->get_slug() . '_parent_type', 'option' );

		return ! empty( $parent_type ) ? $parent_type : Text_Attribute::PARENT_TYPE_VALUES;
	}
