<template>
    <div>
        ...
        <div v-if="state === 'showDemo'">
            <h1>{{ getString('plugin_name') }}</h1>
            <div class="mh-importer__start">
                <div class="mh-importer__start__top" v-html="getString('description')"></div>
                <div class="mh-importer__start__bottom">
                    <p>
                    123 The importer has 2 modes - "standard" and "slow". Switch to "slow" if "standard" fails (it's rare but sometimes hosting do not allow to upload files very fast)
                    </p>
                    <select v-model="mode">
            ...
            <h2>My import failed. What can I do?</h2>
            <div class="mh-importer__php">
                <div class="mh-importer__speed">
                    <div>
                        <div>
                        If importer is not working, your server has some unusual settings. Lets try to find it.
                        </div>
                        <div>
                            <h3>1. Low PHP configuration limit settings</h3>
                            Importer can be blocked by excessively low PHP configuration limit settings of the server. You can try this instruction to increase it:
                        </div>
                        <div>
                        <h4>1.1. Fix via wp-config</h4>
                            <ol>
                                <li>Open the <strong>wp-config.php</strong> file located in the root of your WordPress file directory</li>
                                <li>Backup the file</li>
                                <li>Paste this code to the very bottom, right before the line that says, "That's all, stop editing! Happy blogging."</li>
                                    <pre>define( 'WP_MAX_MEMORY_LIMIT', '256M' );</pre>
                                </li>
                                <li>Save the <strong>wp-config.php</strong> file
                                <li>Run Import again</li>
                            </ol>
                        </div>
                        <div>
                            <h4>1.2. Fix via PHP.ini</h4>
                            If it is still not working you need to edit the PHP.ini file. Please find this lines and change it:
                            <pre class="mh-importer__speed__php-pre">memory_limit = 256M<br>max_execution_time = 300</pre>
                            If you have access to the cPanel you can probably find possibility to edit PHP.ini there.
                        </div>
                        <div>
                            <h3>2. Check mbstring</h3>
                            If it is still now working, check if your server have installed:
                            - <strong>PHP mbstring</strong> library
                        </div>
                        <div>
                            <h3>3. Some Windows servers</h3>
                        ...
                        <div>
                            <h3>4. Server does not create thumbnails</h3>
                            It is a server problem that <strong>PHP module GD</strong> is not installed
                        </div>
                        <div>
                            <h3>5. Support Team - Help</h3>
                            If you need help with importer please contact us via support@tangibledesign.net. Please send us /wp-admin/ login and password and purchase code to verify-purchase ( <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-">Where is my Purchase Code?</a> ) and we will help you.
                        </div>
                    </div>
                </div>
            </div>
